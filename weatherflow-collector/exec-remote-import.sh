...
if [ -z "${threads}" ]; then echo "${echo_bold}${echo_color_remote_import}${collector_type}:${echo_normal} $(date) - ${echo_bold}WEATHERFLOW_COLLECTOR_THREADS${echo_normal} environmental variable not set. Defaulting to ${echo_bold}4${echo_normal} threads."; threads="4"; export WEATHERFLOW_COLLECTOR_THREADS="4"; fi
...
echo "${echo_bold}${echo_color_remote_import}${collector_type}:${echo_normal} $(date) - Starting WeatherFlow Collector (exec-remote-import.sh) - https://github.com/lux4rd0/weatherflow-collector
...
echo -n "${echo_bold}${echo_color_remote_import}${collector_type}:${echo_normal} $(date) - Import Duration: ${echo_bold}"; show_progress_time ${import_duration_seconds}
...
curl_message="weatherflow_system_stats,collector_key=${collector_key},collector_type=${collector_type},duration_type="import",host_hostname=${host_hostname},public_name=${public_name_escaped},source=${function},station_id=${station_id},station_name=${station_name_escaped} duration=${import_duration},import_metrics=${num_of_metrics_minus_one},import_threads=${threads}"
