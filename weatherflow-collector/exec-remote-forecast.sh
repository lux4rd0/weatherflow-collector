...
echo "${echo_bold}${echo_color_remote_forecast}${collector_type}:${echo_normal} Starting WeatherFlow Collector (exec-remote-forecast.sh) - https://github.com/lux4rd0/weatherflow-collector
...
if [ "${day_start_local}" != "null" ]; then curl_message="${curl_message}day_start_local=${day_start_local},"; else echo "${echo_bold}${echo_color_remote_forecast}${collector_type}:${echo_normal} day_start_local is null"; fi
if [ "${icon}" != "null" ]; then curl_message="${curl_message}icon=\"${icon}\","; else echo "${echo_bold}${echo_color_remote_forecast}${collector_type}:${echo_normal} icon is null"; fi
...
if [ "${sunrise}" != "null" ]; then curl_message="${curl_message}sunrise=${sunrise},"; else echo "${echo_bold}${echo_color_remote_forecast}${collector_type}:${echo_normal} sunrise is null"; fi
if [ "${sunset}" != "null" ]; then curl_message="${curl_message}sunset=${sunset}"; else echo "${echo_bold}${echo_color_remote_forecast}${collector_type}:${echo_normal} sunset is null"; fi
...
curl_message="weatherflow_system_stats,collector_key=${collector_key},collector_type=${collector_type},elevation=${elevation},host_hostname=${host_hostname},latitude=${latitude},longitude=${longitude},public_name=${public_name_escaped},source=${function},station_id=${station_id},station_name=${station_name_escaped},timezone=${timezone} forecast_daily_build_duration=${daily_duration}"
...
if [ "$function" == "import" ]; then ProgressBar "${hour}" ${num_of_hours_minus_one}; fi
...
weatherflow_system_stats,collector_key=${collector_key},collector_type=${collector_type},elevation=${elevation},host_hostname=${host_hostname},latitude=${latitude},longitude=${longitude},public_name=${public_name_escaped},source=${function},station_id=${station_id},station_name=${station_name_escaped},timezone=${timezone} forecast_hourly_build_duration=${hourly_duration}"
