...
echo "Starting WeatherFlow Collector (exec-remote-socket.sh) - https://github.com/lux4rd0/weatherflow-collector
...
echo "[${line},${loki_meta}]" | ${grafana_loki_binary_path} --stdin --client.url "${loki_client_url}" --client.external-labels=collector_type="${collector_type}",device_id="${device_id}",source="${function}",hub_sn="${hub_sn}",public_name="${public_name}",serial_number="${serial_number}",station_id="${station_id}",station_name="${station_name}" --config.file=loki-config.yml; fi
...
echo "[${line},${loki_meta}]" | ${grafana_loki_binary_path} --stdin --client.url "${loki_client_url}" --client.external-labels=collector_type="${collector_type}",device_id="${device_id}",source="${function}",public_name="${public_name}",station_id="${station_id}",station_name="${station_name}" --config.file=loki-config.yml; fi
...
echo "[${line},${loki_meta}]" | ${grafana_loki_binary_path} --stdin --client.url "${loki_client_url}" --client.external-labels=collector_type="${collector_type}",device_id="${device_id}",source="${function}",hub_sn="${serial_number}",public_name="${public_name}",serial_number="${serial_number}",station_id="${station_id}",station_name="${station_name}" --config.file=loki-config.yml; fi
