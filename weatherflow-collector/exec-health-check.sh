...
echo "${echo_bold}${echo_color_health_check}${collector_type}:${echo_normal} $(date) - ${health_check_collector_type} check is more than ${health_check_interval} seconds old."; pkill -TERM -P "$(pgrep -f start-"${health_check_collector_type}".sh)"
echo "
...
curl "${curl[@]}" -i -XPOST "${influxdb_url}" -u "${influxdb_username}":"${influxdb_password}" --data-binary "
weatherflow_system_events,collector_key=${collector_key},collector_type=${collector_type},host_hostname=${host_hostname},source=${function},event=healthcheck,health_check_collector_type=${health_check_collector_type} up=0"
echo "${echo_bold}${echo_color_health_check}${collector_type}:${echo_normal} $(date) - ${health_check_collector_type} check has failed. No check file found."; pkill -TERM -P "$(pgrep -f start-"${health_check_collector_type}".sh)"
echo "
${echo_bold}${echo_color_health_check}${collector_type}:${echo_normal} ${health_check_collector_type} restarted."
...
curl "${curl[@]}" -i -XPOST "${influxdb_url}" -u "${influxdb_username}":"${influxdb_password}" --data-binary "
weatherflow_system_stats,collector_key=${collector_key},collector_type=${collector_type},host_hostname=${host_hostname},source=${function} duration=${health_check_duration}"
